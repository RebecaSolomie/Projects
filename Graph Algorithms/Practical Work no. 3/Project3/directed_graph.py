...
class DirectedGraph:
    ...
    def backwards_dijkstra(self, target, source):
        """
        Backwards Dijkstra algorithm for the lowest cost walk between two vertices
        :param target: the target vertex
        :param source: the source vertex
        :return: the lowest cost walk between the two vertices starting from the target
        """
        q = PriorityQueue()  # initialize a priority queue to store vertices
        dist = {}  # initialize a dictionary to store the distances
        next_ = {}  # initialize a dictionary to store the predecessors
        q.put((0, target))  # start with the target vertex
        dist[target] = 0
        while not q.empty():
            cost, x = q.get()
            for y in self.parse_inbound_edges(x):  # explore its neighbors
                new_distance = dist[x] + self.get_endpoints(y, x)
                # extract the vertex with the smallest cost
                if y not in dist.keys() or dist[y] > new_distance:
                    dist[y] = new_distance
                    next_[y] = x
                    q.put((dist[y], y))
                    if y == source:  # if it reached the source vertex
                        path = [y]
                        while y != target:
                            y = next_[y]  # reconstruct the path
                            path.append(y)
                        # path.reverse()  # reverse the path
                        return dist[source], path
        # if source is not reachable from the target
